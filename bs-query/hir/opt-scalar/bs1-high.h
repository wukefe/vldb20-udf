...
static I horse_main(V *h_rtn){
    ...
    // step 1: apply filter
    V filter = incV();
    initV(filter, H_B, vn(sptprice));
    DOP(vn(sptprice), vB(filter,i)=vE(sptprice,i)<50||vE(sptprice,i)>100)
    // step 2: parallelism for boolean selection
    L lenZ = 0, parZ[H_CORE], offset[H_CORE];
    memset(parZ  , 0, sizeof(L)*H_CORE);
    memset(offset, 0, sizeof(L)*H_CORE);
    CHECKE(getNumOfNonZero(filter,parZ));
    DOI(H_CORE, lenZ += parZ[i])
    DOIa(H_CORE, offset[i]=parZ[i-1]+offset[i-1])
    //P("lenZ = %lld\n", lenZ); getchar();
    initV(sptprice1  , H_E, lenZ);
    initV(strike1    , H_E, lenZ);
    initV(rate1      , H_E, lenZ);
    initV(divq1      , H_E, lenZ);
    initV(volatility1, H_E, lenZ);
    initV(time1      , H_E, lenZ);
    initV(otype1     , H_B, lenZ); // boolean
    initV(divs1      , H_E, lenZ);
    initV(dgrefval1  , H_E, lenZ);
    initV(optiontype1, H_C, lenZ);
    DOT(vn(sptprice), if(vB(filter,i)){ L k=offset[tid]++;
            vE(sptprice1  ,k)=vE(sptprice  ,i); 
            vE(strike1    ,k)=vE(strike    ,i); 
            vE(rate1      ,k)=vE(rate      ,i); 
            vE(divq1      ,k)=vE(divq      ,i); 
            vE(volatility1,k)=vE(volatility,i); 
            vE(time1      ,k)=vE(time      ,i); 
            vB(otype1     ,k)=vB(otype     ,i); 
            vE(divs1      ,k)=vE(divs      ,i); 
            vE(dgrefval1  ,k)=vE(dgrefval  ,i);
            vC(optiontype1,k)=vC(optiontype,i);  })
    // step 3: partial results passing
    horse_BlkSchls_opt(tempV, (V []){sptprice1,strike1,rate1,divq1,volatility1,time1,otype1,divs1,dgrefval1}); optionprice = tempV[0];
    PROFILE(11, copyV(columnname, LiteralVectorSymbol(3, (S []){"optiontype", "sptprice", "optionprice"})));
    PROFILE(12, pfnList(columnvalue, 3, (V []){optiontype1,sptprice1,optionprice}));
